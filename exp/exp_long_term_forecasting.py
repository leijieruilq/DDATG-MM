...
class adp(nn.Module):
    def __init__(self, pred_len, device, emb_dim=4, dropout=0.5):
        super().__init__()
    ...
    def forward(self,x):
        ...
        w_x = F.tanh(self.drop(torch.einsum("btl,tm->btm",[x,adj]))) #依赖关系
        np.save("w_x_text.npy",w_x.cpu().detach().numpy())
        # np.save("x_text_eco.npy",x.cpu().detach().numpy())
        # np.save("text_adj_eco.npy",w_x.cpu().detach().numpy())
        # np.save("text_c_emb_eco.npy",self.c_emb.cpu().detach().numpy())
        # np.save("x_text_fusion.npy",(torch.einsum("btm,btl->bml",w_x,x) + x).cpu().detach().numpy())
        return torch.einsum("btm,btl->bml",w_x,x) + x
    
class adp_de(nn.Module):
...
class MLP_g(nn.Module):
    def __init__(self, layer_sizes, pred_len, device, emb_dim=4, dropout_rate=0.5):
        super().__init__()
        self.layers = nn.ModuleList()
        self.dropout = nn.Dropout(dropout_rate)  
        for i in range(len(layer_sizes) - 1):
            self.layers.append(nn.Linear(layer_sizes[i], layer_sizes[i+1]))
        self.graph = nn.ModuleList([adp(pred_len=pred_len,device=device,emb_dim=emb_dim) for i in range(1)])
    def forward(self, x):
...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        if self.args.use_gnn:
            self.mlp = MLP_g(mlp_sizes,pred_len=1024,device=self.device,dropout_rate=0.3)
            self.gnn_text = adp_de(pred_len=self.pred_len,device=self.device)
        else:
        ...
        if self.args.use_gnn:
            self.gnn_text = self.gnn_text.to(self.device)
        self.learning_rate2=1e-2
        self.learning_rate3=1e-3
        self.learning_rate4=1e-4
        self.learning_rate5=1e-4
    ...
    def _select_optimizer_mlp(self):
        ...
        return model_optim
    def _select_optimizer_gnn_text(self):
        model_optim = optim.Adam(self.gnn_text.parameters(), lr=self.args.learning_rate3)
        return model_optim
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                if self.Doc2Vec==False:
                    ...
                    prompt = self.tokenizer(prompt, return_tensors="pt", padding="max_length", truncation=True, max_length=1024).input_ids
                ...
                if self.args.use_gnn:
                    # out_text, x_t = self.gnn_text(prompt_y)
                    outputs=(1-self.prompt_weight)*outputs+self.prompt_weight*prompt_y#out_text
                else:
    ...
    def train(self, setting):
        ...
        if self.args.use_gnn:
            model_optim_gnn_text = self._select_optimizer_gnn_text()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(train_loader):
                ...
                if self.args.use_gnn:
                    model_optim_gnn_text.zero_grad()
                batch_x = batch_x.float().to(self.device)
                ...
                if self.use_fullmodel:
                    ...
                    prompt_emb=prompt_embeddings 
                    #np.save("o_.npy",(torch.mm(prompt_emb[0],prompt_emb[0].T)).cpu().detach().numpy())
                    prompt_emb = self.mlp(prompt_emb)
                # decoder input
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
                ...
                if self.Doc2Vec==False:
                    if self.pool_type=="avg":                
                        global_avg_pool = F.adaptive_avg_pool1d(prompt_emb.transpose(1, 2), 1).squeeze(2) #avg掉所有的token:1024维度
                        prompt_emb=global_avg_pool.unsqueeze(-1)
                    elif self.pool_type=="max":
                        global_max_pool = F.adaptive_max_pool1d(prompt_emb.transpose(1, 2), 1).squeeze(2)
                        prompt_emb=global_max_pool.unsqueeze(-1)
                ...
                batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                if self.args.use_gnn:
                    loss = (1-self.args.alpha)*criterion(outputs, batch_y) + self.args.alpha*(torch.fft.rfft(outputs, dim=1) - torch.fft.rfft(batch_y, dim=1)).abs().mean() 
                else:
                    loss = criterion(outputs, batch_y)
                if self.args.use_gnn:
                    total_loss = loss#/loss.detach() + loss_f/loss_f.detach() #(1-alpha)*loss + alpha*loss_f
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(test_loader):
                ...
                if self.use_fullmodel:
                    ...
                    prompt_emb=prompt_embeddings 
                    np.save("prompt_text.npy",prompt_emb.cpu().detach().numpy())
                    prompt_emb = self.mlp(prompt_emb) 
                # decoder input
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
                ...
                if self.args.use_gnn:
                    # out_text, x_t = self.gnn_text(prompt_y)
                    outputs=(1-self.prompt_weight)*outputs+self.prompt_weight*prompt_y#out_text
                    np.save("time-series-in.npy",batch_x.cpu().detach().numpy())
                    np.save("time-series-out.npy",outputs.cpu().detach().numpy())
                    np.save("text-out.npy",prompt_y.cpu().detach().numpy())
                    # np.save("prompt_y_gnn.npy",prompt_y.cpu().detach().numpy())
                else:
                    outputs=(1-self.prompt_weight)*outputs+self.prompt_weight*prompt_y
                    # np.save("prompt_y.npy",prompt_y.cpu().detach().numpy())
